...
export class ProductListComponent implements OnInit {
  ...
  originalLanguageColumns: TableColumn[] = [];
  originalData: any[] = [];
  keyMappings: { [key: string]: string } = {}; // Mapa de claves traducidas a originales
  ...
  /**
   * Alternar entre el idioma original y el español para las etiquetas de columnas y datos
   */
  toggleTranslation(): void {
    ...
    if (!this.isTranslated) {
      ...
      // Guardar los datos originales si es la primera vez
      if (this.originalData.length === 0) {
        this.originalData = JSON.parse(JSON.stringify(this.productos));
      }
      
      // Crear un mapa inverso de claves (original -> traducida)
      this.keyMappings = {};
      Object.entries(keyTranslations).forEach(([original, translated]) => {
        this.keyMappings[original] = translated;
      });
      
      // Traducir las etiquetas y claves de columnas
      ...
      // Transformar los datos para usar las claves traducidas
      this.productos = this.productos.map(producto => {
        const translatedProduct: any = {};
        
        // Copiar todas las propiedades originales y sus valores a las claves traducidas
        Object.entries(producto).forEach(([key, value]) => {
          const translatedKey = keyTranslations[key] || key;
          translatedProduct[translatedKey] = value;
          
          // Mantener también las claves originales para compatibilidad
      ...
      if (this.originalData.length > 0) {
        this.productos = JSON.parse(JSON.stringify(this.originalData));
      }
